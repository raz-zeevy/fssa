...
from lib.components.form import SelectionBox
...
class EditableTreeview(ttk.Treeview):
    ...
    def _on_click(self, event):
        ...
        if (row_id != self._cur_focus or column_id != self._cur_focus_col) \
                and self._entry_popup:
                self._on_return(self._cur_focus, self._cur_focus_col,
                                self._entry_popup)
                self._entry_popup = None
                self._cur_focus = None
                self._cur_focus_col = None
    ...
    def _enter_edit_mode(self, row_id, column_id):
        if self._cur_focus_col == "#0":  # Skip the first column with
            # checkboxes
            return
        self._entry_popup = tk.Entry(self)
        self._entry_popup.insert(0, self.item(row_id, 'values')[
            int(self._cur_focus_col[1:])-1])
        self._entry_popup.select_range(0, tk.END)
        self._entry_popup.bind("<Return>", lambda e: self._on_return(row_id,
                                                                     column_id,
                                                                     self._entry_popup))
        self._entry_popup.bind("<KP_Enter>", lambda e: self._on_return(row_id,
                                                                       column_id,
                                                                       self._entry_popup))
        self._entry_popup.bind("<Escape>", lambda e: self._entry_popup.destroy())
        self._entry_popup.bind("<FocusOut>", lambda e: self._on_return(row_id,
                                                                       column_id,
                                                                       self._entry_popup))  # Destroy the popup when focus is lost
    ...
    def _on_check_click(self, event):
        ...
        self.item(row_id, tags=('checkbox',))
