...
class DiagramWindow(Window):
    ...
    def create_menu(self):
        ...
        self.file_menu.add_command(label="Save figure ", command=self.save_figure)
        # add an option to save all figures
        self.file_menu.add_command(
            label="Save all figures", command=self.save_all_figures
        )
    ...
    def plot_scatter(self, graph_data):
        def add_geoms(x, axes, graph_data):
            def add_line(x, axes, line: Line):
                ...
                start, end = min(x) * 0.5, max(x) * 2  # buffers to ensure it is
                # long enough
                start, end = start - (end - start) * OC, end + (end - start) * OC
            ...
            def add_circle(axes, circle: Circle):
                ...
                circle_plot = matplotlib.patches.Circle(
                    circle.center, circle.radius, edgecolor=G_COLOR, facecolor="none"
                )
    ...
    def create_bottom_panel(self):
        ...
        frame_navigation.pack(
            side=ttk.BOTTOM, fill="x", padx=real_size(10), pady=real_size((0, 40))
        )
        ...
        self.button_save_figure = NavigationButton(
            center_frame,
            text="Save Figure",
            command=self.save_figure,
        )
        self.button_save_figure.pack(side=ttk.LEFT, padx=real_size(20))
        self.button_exit = NavigationButton(
            center_frame,
            text="Exit",
            bootstyle="secondary",
            command=self.exit,
...
def test_diagram_window(n=35):
    ...
    legend_items = [
        {"index": i + 1, "value": f"Variable {annotations[i]}"}
...
if __name__ == "__main__":
    ...
    window1 = test_diagram_window(n=3)
    window1.mainloop()
